...
import { getCookie, getHeaders, setCookie } from 'vinxi/http';
...
export const getCurrentSession = async (): Promise<SessionValidationResult> => {
  const token =
    getHeaders().Authorization?.split('Bearer ')[1] ??
...
export async function refreshSession(session: Session): Promise<void> {
  if (
    ...
    session.expiresAt.getTime() -
      (1000 * envServer.SESSION_EXPIRATION_SECONDS) / 2
  ) {
    session.expiresAt = new Date(
      Date.now() + 1000 * envServer.SESSION_EXPIRATION_SECONDS
    );
