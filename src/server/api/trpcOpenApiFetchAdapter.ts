...
const createMockNodeHTTPRequest = async (
  srcReq: Request,
  url: URL
): Promise<NodeHTTPRequest> => {
...
export function createOpenApiFetchHandler<TRouter extends OpenApiRouter>(
  ...
  return async (srcReq: Request): Promise<Response> => {
    ...
    const url = new URL(
      srcReq.url.startsWith('/') ? `http://127.0.0.1${srcReq.url}` : srcReq.url
    );
    ...
    try {
      ...
      const statusCode =
        meta?.status ?? getHTTPStatusCodeFromError(error) ?? 500;
